...
class Transformer(nn.Module):
    ...
    def __greedy_decode(
        ...
        for _ in range(max_length):
            ...
            logits = dec_output  # Shape: (batch_size, vocab_size)
    ...
    def __beam_search_decode(
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):  # DO NOT CHANGE
                    new_seq = torch.cat(
                        (seq, torch.tensor([[next_token]], device=device)), 1
                    )  # Shape: (1, seq_len+1)
                    new_score = log_probs[:, next_token]
        ...
        **kwargs,
        ...
        for _ in range(max_length):
            ...
            dec_outputs_candidate = self.decoder(
                y_candidates, enc_output.expand(k, -1, -1)
            )
            ...
            cos_sim = (h_v_norm.unsqueeze(1) * h_j_norm).sum(
                dim=-1
            )  # Shape: (k, seq_len)
